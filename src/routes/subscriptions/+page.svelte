	  ...
	  <section>
		...
		<div class="flex flex-wrap gap-6">
		  {#each product.prices as price }
			{@const subscriptionByPrice = data.currentSubscriptions.find(item => item.priceId == price.id)}
			...
			<div class="bg-white shadow-lg p-4 rounded-md w-full sm:w-auto flex-shrink-0">
			  ...
			  {#if subscriptionByPrice}
				{#if subscriptionByPrice.canceled?.isCanceled == false}
				  <NavButton text="Cancelar" href={relativeUrls.subscriptions.updateSuscription(subscriptionByPrice.subscriptionId, true)}/>
				{:else}
				  <NavButton text="Resuscribirse" href={relativeUrls.subscriptions.updateSuscription(subscriptionByPrice.subscriptionId, false)}/>
				  {@const canceledDate = flatMap(subscriptionByPrice.canceled?.cancelAt, cancelAt => new Date(cancelAt * 1000))}
				  {#if canceledDate}
					<p>La suscripción se cancelará el {canceledDate.toLocaleDateString()}</p>
				  {/if}
				{/if}
			  {:else if data.currentSubscriptions.length}
				<NavButton text="Cambiar" href={relativeUrls.subscriptions.portalUpdateSubscription(data.currentSubscriptions[0].subscriptionId)}/>
			  {:else}
